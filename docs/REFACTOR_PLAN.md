...
| 类型 | 数量 | 明细与建议 |
|---|---|---|
| shared_cid | 6 | 3 组重复行共用目录：Gaslight 本体/パック（共用磁链）、魔法騎士リーンフォリア 1代/1+2、顔のない月 同月表记差异 → 建议合并行 |
| ambiguous | 5 | 同名/近似目录多候选：恋愛、はじめまして / 僕に抱かれ喘ぐ妻…2 / 猫忍えくすはーとSPIN！2 / ユニオリズム B2-STYLE / ストレイ・シープ Vol.1 → 人工指定目录后 `--name` 单独整理 |
| not_dir | 1 | ムスコに魅せられて（单文件种子） |
| no_dn_date | 1 | ディメンション凸ラバース！！（英文版无日期码） |
