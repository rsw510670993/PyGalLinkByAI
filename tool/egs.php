...
(function () {
    ...
    function kindLabel(value) {
        if (value === 'CIRCLE') return '社团';
        if (value === 'CORPORATION') return '企业';
        return value || '';
    }
    ...
    function render(data) {
        ...
        if (!data.data || data.data.length === 0) {
            tbody.innerHTML = '<tr><td colspan="4" class="text-center text-muted py-4">没有数据</td></tr>';
        } else {
            tbody.innerHTML = data.data.map(row => `
                <tr>
                    ...
                    <td>${esc(kindLabel(row.brand_kind))}</td>
                </tr>
            `).join('');
        }
    ...
    async function load() {
        const tbody = document.getElementById('games-body');
        tbody.innerHTML = '<tr><td colspan="4" class="text-center text-muted py-4">加载中...</td></tr>';
        try {
            ...
            tbody.innerHTML = `<tr><td colspan="4" class="text-center text-danger py-4">加载失败：${esc(err.message)}</td></tr>`;
        }
    }
